...
- **Reloads** are `WidgetBridge.reloadAllTimelines()` — the app's only `WidgetKit` import — from three callers: backgrounding, `AppDelegate.didReceiveRemoteNotification` (the §5 quiet push), and `publishWidgetSnapshot` whenever a CHANGED file lands while the app is in the background. Foreground stays rationed: §5-A's budget is ~40–70 a day and nobody is looking at a widget behind an open app.
